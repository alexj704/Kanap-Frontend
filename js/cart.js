...
if (inLocalStorage !== null && inLocalStorage.length > 0) {
  for (let i in inLocalStorage) {
    fetch(
      `https://kanap-backend-k6sk.onrender.com/api/products/${inLocalStorage[i].productId}`
    )
      ...
      .then((data) => {
        ...
        document.getElementById(
          "cart__items"
        ).innerHTML += `<article class="cart__item" data-id="${inLocalStorage[i].productId}" data-color="${inLocalStorage[i].color}">
...
function quantityChange() {
  ...
  items.forEach((item) => {
    ...
    item.addEventListener("change", function (event) {
      const change = inLocalStorage.find(
        (element) => element.productId == dataId && element.color == dataColor
      );
      if (event.target.value < 1 || event.target.value > 100) {
        alert(
          "Veuillez sélectionner entre 1 et 100 produits, pour supprimer un produit, cliquez sur Supprimer"
        );
      } else {
        change.quantity = Number(event.target.value);
        window.localStorage.setItem(
          "Products",
          JSON.stringify(getProductLocalStorage())
        );
        priceTotal();
      }
    });
...
function deleteItem() {
  ...
  deleteItems.forEach((deleteItem) => {
    ...
    deleteItem.addEventListener("click", (event) => {
      const deleted = inLocalStorage.find(
        (element) => element.productId == dataId && element.color == dataColor
      );
      inLocalStorage = inLocalStorage.filter(
        (deleteItem) => deleteItem !== deleted
      );
...
function validAddress(inputAddress) {
  ...
  if (
    /^[0-9a-záàâäãåçéèêëíìîïñóòôöõúùûüýÿæœ.\s-]+$/i.test(inputAddress.value)
  ) {
    return true;
  } else {
...
form.addEventListener("submit", (event) => {
  ...
  // Sinon on vérifie si les champs sont correctement remplis
    ...
    validNameOrCity(form.firstName) &&
    validNameOrCity(form.lastName) &&
    validAddress(form.address) &&
    validNameOrCity(form.city) &&
    ...
    fetch("https://kanap-backend-k6sk.onrender.com/api/products/order", {
      method: "POST",
      headers: {
