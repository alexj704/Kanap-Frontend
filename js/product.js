...
fetch(`https://kanap-backend-k6sk.onrender.com/api/products/${id}`)
  .then((response) => response.json())
  // Création des éléments du DOM et ajout des infos venant de fetch
  .then((data) => {
    // Changement du titre de la page selon le modèle
    document.querySelector("title").textContent = `${data.name} - Kanap`;
...
addToCart.addEventListener("click", function (event) {
  ...
  if (
    ...
    productOptions.quantity > 100 ||
    productOptions.color === ""
  ) {
    window.alert(
      "Veuillez choisir une couleur et entrer une quantité entre 1 et 100"
    );
  } else {
...
function addLocalStorage(productOptions) {
  ...
  if (inLocalStorage === null) {
    ...
    const found = inLocalStorage.find(
      (element) =>
        element.productId == productOptions.productId &&
